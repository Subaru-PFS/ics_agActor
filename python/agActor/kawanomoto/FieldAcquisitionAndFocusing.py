...
class PFS():
    ...
    def FAinstpa(
        ...
        v_0, v_1 = pfs.makeBasis(tel_ra, tel_de, catalog_array[:, 0], catalog_array[:, 1], dt, adc, inr, m2pos3, wl)

        v_0 = (np.insert(v_0, 2, catalog_array[:, 2], axis=1))
        v_1 = (np.insert(v_1, 2, catalog_array[:, 2], axis=1))

        # source filtering
        filtered_detections, v = pfs.sourceFilter(detected_array, maxellip, maxsize, minsize)

        ra_offset, de_offset, inr_offset, scale_offset, mr, md = pfs.RADECInRScaleShift(
            filtered_detections[:, 2],
            filtered_detections[:, 3],
            filtered_detections[:, 4],
            filtered_detections[:, 7],
            v_0, v_1
            )
