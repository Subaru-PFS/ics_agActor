...
class telCenter:
    ...
    def __init__(self, *, actor=None, logger=None, center=None, design=None, tel_dither=None, tel_guide=None, tel_status=None):

        self._actor = actor
        self._logger = logger if logger is not None else actor.logger if actor is not None else None
        self._center = center if center is not None else pfs_design(design_id=design[0], design_path=design[1], logger=actor.logger).center if design is not None else None
    ...
    def center(self):
        ...
        if self._center is not None:
            ...
            ra, dec, inst_pa = tel_status[5:8]  # command ra, dec, and inst_pa (including dither offsets and guide offsets)
            ra -= (tel_guide['ra'] + tel_dither['ra'] / float(numpy.cos(numpy.deg2rad(dec - tel_guide['dec'] / 3600)))) / 3600
    ...
    def dither(self):
        ...
        if self._center is not None:
            ...
            dither = self._center[0] + tel_dither['ra'] / float(numpy.cos(numpy.deg2rad(dec))) / 3600, dec, self._center[2] + tel_dither['pa'] / 3600
        else:
            tel_status = self._tel_status if self._tel_status is not None else self._actor.gen2.tel_status
            self._logger and self._logger.info('tel_status={}'.format(tel_status))
            dither = tel_status[5] - tel_guide['ra'] / 3600, tel_status[6] - tel_guide['dec'] / 3600, tel_status[7] - tel_guide['insrot'] / 3600
    ...
    def offset(self):
        ...
        if self._center is not None:
            ...
            self._logger and self._logger.info('tel_dither={}'.format(tel_dither))
            dec = self._center[1] + tel_dither['dec'] / 3600
            offset = tel_dither['ra'] / float(numpy.cos(numpy.deg2rad(dec))), tel_dither['dec'], tel_dither['pa'], - tel_guide['insrot']
        else:
