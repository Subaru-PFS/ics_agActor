...
def set_design_agc(*, frame_id=None, obswl=0.62, logger=None, **kwargs):
    ...
    if frame_id is not None:
        ...
        guide_objects = astrometry.measure(
            detected_objects=detected_objects, ra=ra, dec=dec, obstime=taken_at, inst_pa=inst_pa, adc=adc,
        ...
        if design_path is not None:
            ...
            logger and logger.info(
                'taken_at={},inr={},adc={},m2_pos3={},magnitude={}'.format(taken_at, inr, adc, m2_pos3, magnitude)
                )
            if 'dra' in kwargs:
                ra += kwargs.get('dra') / 3600
            if 'ddec' in kwargs:
...
def autoguide(*, frame_id, obswl=0.62, logger=None, **kwargs):
    ...
    return (ra, dec, inst_pa, *field_acquisition._acquire_field(
        guide_objects=guide_objects, detected_objects=detected_objects, ra=ra, dec=dec, taken_at=taken_at, adc=adc,
        inst_pa=inst_pa, m2_pos3=m2_pos3, obswl=obswl, altazimuth=True, logger=logger, **_kwargs
